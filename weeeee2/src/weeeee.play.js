...
//		us.plx=gamecake.gfx.parallax({parent:us.sheet,sx:us.hx,sy:us.hy,hx:us.hx*2,hy:us.hy*2}).draw();
		...
		us.p[9]=gamecake.gfx.sheet({parent:us.sheet,name:"over",px:320,py:0}).draw();

		us.p[10]=gamecake.gfx.sheet({parent:us.sheet,name:"score"}).draw();
		us.p[11]=gamecake.gfx.text({parent:us.sheet,px:100,py:6,sx:100,sy:25,align:"right",size:16}).draw();
//		us.p[11].align("right");
//		us.p[11].size(32);
...
//		gamecake.sfx.audio({name:"start"});
	...
	us.update=function()
	{
		...
		{
			if(speed<1) { us.p[9].px-=1; } //make sure we get onto the screen
			else
			{
		...
		if(us.state=="dead") // clicky
		{
			...
			{
				if(gamecake.input.y<150)// submit score
				{
